...
// Funcion auxiliar para subir directorios enteros de forma recursiva a Backblaze B2
async function uploadFolderToB2(localDirPath, b2FolderPath) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (entry.name.endsWith('.m3u8')) mimeType = 'application/vnd.apple.mpegurl'; else if (entry.name.endsWith('.ts')) mimeType = 'video/MP2T'; else if (entry.name.endsWith('.jpg') || entry.name.endsWith('.jpeg')) mimeType = 'image/jpeg'; else if (entry.name.endsWith('.vtt')) mimeType = 'text/vtt';
...
export function createWorker() {
    ...
    try {
        ...
        await updateVideoProcessing({
            ...
            originalPath: newRawPath,
            originalFileName: path.basename(s3KeySegura)
        });
        ...
        const folderDestinoB2 = `Videos/HLS/${videoId}`;
        console.log(`[Worker] Subiendo HLS a ${folderDestinoB2}`);
        await uploadFolderToB2(hlsDir, folderDestinoB2);
        await uploadFolderToB2(thumbDir, `Videos/Thumbnails/${videoId}`);
        await updateVideoProcessing({
            status: 'completed',
            progress: 100,
            hlsFolder: folderDestinoB2,
            masterPlaylist: 'master.m3u8',
            thumbnailPath: `Videos/Thumbnails/${videoId}/thumbnail.jpg`,
            previewVttPath: `Videos/Thumbnails/${videoId}/preview.vtt`,
            availableQualities,
